...
所有真实运行均通过同源 `/api/qianscope/*` 网关访问 FastAPI，DeepSeek Key 只存在于 API 服务端。工具创建后台任务后显示任务 ID、真实阶段、已处理 Agent 数和最新轨迹；支持终止，并能按任务 ID 恢复持久化结果。报告、事件、城市、剧本和人物访谈会显示本次模型或调用回执。后端不可用时，界面只会显示明确标注为“合成演示 · 未校准”的回退内容。
