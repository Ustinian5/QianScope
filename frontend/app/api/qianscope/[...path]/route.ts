...
function backendUrl(path: string) {
  const configured = process.env.QIANSCOPE_API_URL
    || process.env.ECHO_API_URL
    || (process.env.NODE_ENV === 'development' ? 'http://127.0.0.1:8000' : '');
  if (!configured) {
    throw new Error('QIANSCOPE_API_URL is not configured for this deployment.');
  }
...
async function proxy(request: Request, context: Context) {
  ...
  try {
    ...
    return Response.json(
      {
        detail: 'QianScope backend is unavailable.',
        hint: 'Start the FastAPI service or configure QIANSCOPE_API_URL.',
        cause: message,
      },
      { status: 503 },
    );
